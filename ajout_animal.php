...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $sql = "INSERT INTO animaux (nom, espece, alimentation, image, paysorigine, descriptioncourte, id_habitat) 
            VALUES (:nom, :espece, :alimentation, :image, :paysorigine, :descriptioncourte, :id_habitat)";
...
<body class="bg-gray-100 min-h-screen p-6">
    ...
    <div class="max-w-2xl mx-auto">
        <h1 class="text-3xl font-bold mb-6">Ajouter un animal</h1>
        ...
        <form method="POST" class="bg-white rounded-lg shadow p-6">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="espece" required class="w-full border rounded px-3 py-2">
            </div>

            <div class="mb-4">
                <label class="block font-semibold mb-2">Alimentation</label>
                <input type="text" name="alimentation" required class="w-full border rounded px-3 py-2">
            </div>

            <div class="mb-4">
                <label class="block font-semibold mb-2">Pays d'origine</label>
                <input type="text" name="paysorigine" required class="w-full border rounded px-3 py-2">
            </div>

            <div class="mb-4">
                <label class="block font-semibold mb-2">Image (nom du fichier)</label>
                <input type="text" name="image" placeholder="lion.jpg" required class="w-full border rounded px-3 py-2">
            </div>

            <div class="mb-4">
                <label class="block font-semibold mb-2">Description courte</label>
                <textarea name="descriptioncourte" required class="w-full border rounded px-3 py-2" rows="3"></textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="id_habitat" required class="w-full border rounded px-3 py-2">
                    ...
                    <?php foreach($habitats as $h): ?>
                        <option value="<?= $h->getIdHab() ?>"><?= $h->getNomHab() ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
